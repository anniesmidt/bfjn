...
S2.define('select2wpupg/results',[
  ...
  Results.prototype.displayMessage = function (params) {
    ...
    var $message = $(
      ...
      ' class="select2wpupg-results__option"></li>'
    );
    ...
    $message[0].className += ' select2wpupg-results__message';

    this.$results.append($message);
  };

  Results.prototype.hideMessages = function () {
    this.$results.find('.select2wpupg-results__message').remove();
  };
...
S2.define('select2wpupg/selection/multiple',[
  ...
  MultipleSelection.prototype.bind = function (container, $container) {
    ...
    this.$selection.on(
      'click',
      '.select2wpupg-selection__choice__remove',
      function (evt) {
...
S2.define('select2wpupg/dropdown/infiniteScroll',[
  ...
  InfiniteScroll.prototype.createLoadingMore = function () {
    var $option = $(
      '<li ' +
      'class="select2wpupg-results__option select2wpupg-results__option--load-more"' +
...
S2.define('select2wpupg/core',[
  ...
  Select2wpupg.prototype.hasFocus = function () {
    return this.$container.hasClass('select2wpupg-container--focus');
  };

  Select2wpupg.prototype.focus = function (data) {
    // No need to re-trigger focus events if we are already focused
    if (this.hasFocus()) {
      return;
    }

    this.$container.addClass('select2wpupg-container--focus');
    this.trigger('focus', {});
  };
...
S2.define('select2wpupg/compat/utils',[
  'jquery'
], function ($) {
  function syncCssClasses ($dest, $src, adapter) {
    ...
    if (classes) {
      ...
      $(classes.split(/\s+/)).each(function () {
        // Save all Select2wpupg classes
        if (this.indexOf('select2wpupg-') === 0) {
    ...
    if (classes) {
      ...
      $(classes.split(/\s+/)).each(function () {
        // Only adapt non-Select2wpupg classes
        if (this.indexOf('select2wpupg-') !== 0) {
...
S2.define('select2wpupg/compat/containerCss',[
  'jquery',
  './utils'
...
S2.define('select2wpupg/compat/dropdownCss',[
  'jquery',
  './utils'
], function ($, CompatUtils) {
  // No-op CSS adapter that discards all classes by default
...
S2.define('select2wpupg/compat/initSelection',[
  'jquery'
], function ($) {
  function InitSelection (decorated, $element, options) {
    if (options.get('debug') && window.console && console.warn) {
      console.warn(
        'Select2wpupg: The `initSelection` option has been deprecated in favor' +
        ' of a custom data adapter that overrides the `current` method. ' +
        'This method is now called multiple times instead of a single ' +
        'time when the instance is initialized. Support will be removed ' +
        'for the `initSelection` option in future versions of Select2wpupg'
      );
    }
...
S2.define('select2wpupg/compat/inputData',[
  'jquery'
], function ($) {
  function InputData (decorated, $element, options) {
    ...
    if ($element.prop('type') === 'hidden') {
      if (options.get('debug') && console && console.warn) {
        console.warn(
          'Select2wpupg: Using a hidden input with Select2wpupg is no longer ' +
...
S2.define('select2wpupg/compat/matcher',[
  'jquery'
], function ($) {
...
S2.define('select2wpupg/compat/query',[

], function () {
  function Query (decorated, $element, options) {
    if (options.get('debug') && window.console && console.warn) {
      console.warn(
        'Select2wpupg: The `query` option has been deprecated in favor of a ' +
...
S2.define('select2wpupg/dropdown/attachContainer',[
  ...
  AttachContainer.prototype.position =
    ...
    $dropdown.addClass('select2wpupg-dropdown--below');
    $container.addClass('select2wpupg-container--below');
  };

  return AttachContainer;
});

S2.define('select2wpupg/dropdown/stopPropagation',[
...
S2.define('select2wpupg/selection/stopPropagation',[
...
], function ($, _, Select2wpupg, Defaults) {
